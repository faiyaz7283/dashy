...
**Decision logic:**
- Used by only one feature? → `features/<feature>/components/`
- Used by 2+ features? → `shared/components/`
- Calendar views (DayView, WeekGrid, etc.) go in `features/calendar/views/` not `components/`
...
```typescript
// features/<feature>/components/<ComponentName>/index.ts
...
```typescript
// features/<feature>/components/<ComponentName>/<ComponentName>.test.tsx
...
```typescript
// features/<feature>/views/<ViewName>/<ViewName>.tsx
...
```
features/calendar/components/TaskItem/
├── TaskItem.tsx
├── TaskItem.test.tsx
└── index.ts
```
