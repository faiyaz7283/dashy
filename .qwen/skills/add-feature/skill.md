...
If the feature has domain-specific types, add them to `domain/<featureName>/`:
...
```typescript
// features/tasks/views/TaskListView/TaskListView.tsx
/**
...
```typescript
// features/dashboard/AppShell/AppShell.tsx
...
If the feature needs navigation, update `shared/config/navigation.ts`:
...
- **Domain types**: Add to `domain/<featureName>/` if feature has domain logic
