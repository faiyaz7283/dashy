...
- Review existing tokens in `theme/tokens.ts`

## Steps

### 1. Determine token category

Tokens are organized into categories in `theme/tokens.ts`:
...
Edit `theme/tokens.ts` and add the token to the appropriate category:
...
If the token needs semantic meaning or user-overridability, add to `theme/config.ts`:
...
- [ ] Token added to `theme/tokens.ts` in appropriate category
- [ ] Token has JSDoc comment describing purpose
- [ ] Token uses `as const` for type safety
- [ ] Token added to `theme/config.ts` (if semantic/configurable)
...
```typescript
// features/dashboard/components/AlertBanner/AlertBanner.tsx
