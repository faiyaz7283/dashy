...
- Check if lucide-react has the icon: `grep -r "from 'lucide-react'" frontend/src/` to see what's already imported
...
```
features/<feature>/components/<ParentComponent>/icons/
├── IconName.tsx
└── (more icons...)
```
...
```
features/weather/components/WeatherTooltip/icons/
...
features/weather/components/WeatherWidget/
└── WeatherIcon.tsx        # 15 OWM conditions with day/night variants

features/calendar/components/EventItem/
...
grep -rh "from 'lucide-react'" frontend/src/ | sort -u
```
...
Create `features/<feature>/components/<ParentComponent>/icons/<IconName>.tsx`.
...
- [ ] Quality gate passes (`make lint && make typecheck && make build`)

## Example: Adding a DewPointIcon to the weather tooltip

1. Check lucide-react — no dew point icon exists there
2. Create `features/weather/components/WeatherTooltip/icons/DewPointIcon.tsx`
...
3. Create `features/calendar/components/EventItem/MapPinIcon.tsx`
