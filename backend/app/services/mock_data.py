...
def get_mock_week_calendar(
    ...
    # Event templates: (day_offset, start_h, start_m, end_h, end_m, title, members, all_day, organizer, description, location, recurrence)
    event_templates = [
        (
            0, 9, 0, 9, 30, "Team Standup", ["faiyaz"], False, "faiyaz",
            "Daily sync with the team", None, "RRULE:FREQ=DAILY;BYDAY=MO,TU,WE,TH,FR"
        ),
        (
            0, 10, 0, 11, 0, "Morning Yoga", ["trisha"], False, "trisha",
            "Vinyasa flow class", "Local Studio", None
        ),
        (
            0, 16, 0, 17, 30, "Soccer Practice", ["arya"], False, "faiyaz",
            "Weekly soccer practice", "Community Field", "RRULE:FREQ=WEEKLY;BYDAY=MO"
        ),
        (
            1, 8, 0, 9, 0, "Dentist Appt", ["faiyaz", "arya"], False, "faiyaz",
            "Regular checkup for Arya", "Dr. Smith's Office", None
        ),
        (
            1, 9, 0, 12, 0, "Preschool", ["raya"], False, "trisha",
            "Morning preschool session", "Little Learners Academy", "RRULE:FREQ=WEEKLY;BYDAY=TU,TH"
        ),
        (
            1, 11, 0, 12, 0, "Grocery Shopping", ["trisha"], False, "trisha",
            "Weekly grocery run", "Whole Foods", None
        ),
        (
            1, 18, 0, 19, 0, "Gym", ["faiyaz"], False, "faiyaz",
            "Workout session", "LA Fitness", "RRULE:FREQ=WEEKLY;BYDAY=TU"
        ),
        (
            2, 15, 0, 16, 0, "Reading Club", ["arya"], False, "trisha",
            "Book discussion group", "Library", None
        ),
        (
            2, 16, 0, 17, 0, "Piano Lesson", ["trisha", "arya"], False, "trisha",
            "Arya's piano lesson", "Music School", "RRULE:FREQ=WEEKLY;BYDAY=WE"
        ),
        (
            2, 19, 0, 22, 0, "Date Night", ["faiyaz", "trisha"], False, "faiyaz",
            "Weekly date night", "Italian Restaurant", None
        ),
        (
            3, 10, 0, 12, 0, "Playdate w/ Lily", ["raya"], False, "trisha",
            "Playdate with Lily from preschool", "Lily's House", None
        ),
        (
            3, 17, 0, 18, 30, "Cook Dinner", ["trisha"], False, "trisha",
            "Prepare meals for the week", "Home", None
        ),
        (
            4, 0, 0, 23, 59, "Science Fair Project", ["arya", "faiyaz"], True, "faiyaz",
            "Work on Arya's science fair project", "Home", None
        ),
        (
            4, 13, 0, 17, 0, "Team Offsite", ["faiyaz"], False, "faiyaz",
            "Quarterly team offsite meeting", "Conference Center", None
        ),
        (
            4, 19, 0, 21, 0, "Family Movie Night", ["faiyaz", "trisha", "arya", "raya"], False, "faiyaz",
            "Watch a family movie together", "Home", "RRULE:FREQ=WEEKLY;BYDAY=FR"
        ),
        (
            5, 10, 0, 12, 0, "Park Visit", ["raya", "arya"], False, "trisha",
            "Visit the playground", "Central Park", None
        ),
        (
            5, 11, 0, 13, 0, "Brunch w/ Friends", ["trisha", "faiyaz"], False, "trisha",
            "Brunch with Sarah and Mike", "Cafe Downtown", None
        ),
        (
            6, 11, 0, 13, 0, "Meal Prep", ["faiyaz", "trisha"], False, "trisha",
            "Prepare meals for the upcoming week", "Home", "RRULE:FREQ=WEEKLY;BYDAY=SU"
        ),
        (
            6, 15, 0, 17, 0, "Homework Catch-up", ["arya"], False, "arya",
            "Finish homework assignments", "Home", None
        ),
    ]

    events = []
    for idx, (day_offset, sh, sm, eh, em, title, members, all_day, organizer, description, location, recurrence) in enumerate(event_templates, 1):
