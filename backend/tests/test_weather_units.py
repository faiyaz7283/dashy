...
async def test_weather_endpoint_imperial_units():
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.get("/api/weather?units=imperial")
        assert response.status_code == 200
        data = response.json()
...
async def test_weather_endpoint_metric_units():
    ...
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.get("/api/weather?units=metric")
        assert response.status_code == 200
        data = response.json()
...
async def test_weather_endpoint_invalid_units():
    """Test weather endpoint with invalid units defaults to imperial."""
    transport = ASGITransport(app=app)
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        response = await client.get("/api/weather?units=invalid")
        assert response.status_code == 200
        data = response.json()

        temp = data["current"]["temperature"]
        assert isinstance(temp, (int, float))
        assert temp > 50  # Should default to Fahrenheit
...
async def test_weather_forecast_units():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        # Metric request
        response_metric = await client.get("/api/weather?units=metric")
        data_metric = response_metric.json()

        # Imperial request
        response_imperial = await client.get("/api/weather?units=imperial")
